...
{
    // GopnikCoin: check prefix
    if(uri.scheme() != QString("GopnikCoin"))
        return false;
...
{
    // Convert GopnikCoin:// to GopnikCoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("GopnikCoin://"))
    {
        uri.replace(0, 12, "GopnikCoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "GopnikCoin.lnk";
}
...
{
    return GetAutostartDir() / "GopnikCoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("GopnikCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  GopnikCoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("GopnikCoin-Qt"));
